...
<cfcomponent accessors="true" singleton  threadSafe="true" >
	...
	<cfproperty name="dao" inject="reportesAdhoc/DAO_ConjuntosDatos">
...
<cffunction name="obtenerConjuntoPorId" hint=" Obtiene la informacion de un conjunto y construye el objeto de dominio">
		...
		<cfscript>
			...
			if(!isNull(conjuntoC)){
				...
				var conjunto=populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/ConjuntoDatos"),consultaConjunto);
				var columnas=[];
				...
				for(var x=1; x lte infoCols.recordcount; x++){
					...
					var columna=populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Columna"),infoCols,x);
					...
					for(var i=1; i lte filtrosCol.recordcount; i++){
						var filtro=	populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Filtro"),filtrosCol,i);
						arrayAppend(filtrosN, filtro);
					}


					for(var i=1; i lte agregacionesCol.recordcount; i++){
						var agregacion=	populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Agregacion"),agregacionesCol,i);
