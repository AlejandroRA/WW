...
function obtenerDatos(tipoGrafica, dimGrafica, config){
    ...
    if (tipoReporte == 5) {
        ...
        .done(function(data) {
            ...
            if(data[0].length == 0){
                ...
                // Incluir imagen cuando no hay datos                
                $('<img />').attr('src', '/includes/img/reportesEstrategicos/noData.png')
                ...
                t_operacion = $('#operacion option:selected').attr('data-tittle');
                 $('.graphTitle').text('REPORTE DE ' + '<cfoutput>#Session.cbstorage.conjunto.TITULO#</cfoutput>');
    ...
    if(tipo == 0){
        ...
        for (var j = 0; j < q[0].length; j++) {
            ...
            for (var i = 0; i < q.length-1; i++){
            	...
            	if(typeof valCel === 'number'){
            		valCel = format2(valCel, "$");
            	}
...
function getDataset(conjuntosDatos){
    var totalValues = [];
    var datasets = [];
    var contEltos = 0;
...
function createChart(datos, titulo, subtitulo, tipo, dim, reporte, config){
...
//Variables globales
var multi, tipoGrafica, chart, num_reporte, dimGrafica;
...
$(document).ready(function() {
    ...
    $("#depen").on("select2:select", ".multipleSelect", function (e) { getDependencies(this); });
    $("#depen").on("select2:unselect", ".multipleSelect", function (e) { getDependencies(this); });
    ...
    $('#buttonConfig2').on('click', function () {
        if ($('#formSave').valid()) {
            ...
            $(".opc").each(function(){
                if($(this).val() != null){
                    filtros.push({ name: $(this).attr("id"), value: $(this).val(), tipo: $(this).closest('div .tipo').attr('data-type')});
                }
            });
    ...
    window.actionEvents = {
        ...
        'click .edit-sr': function (e, value, row, index) {
            ...
            searchReport(num_reporte, row.desc);
    ...
    function searchReport(option, txt){
        $.post('consultaVistaMat/cambiarConfiguracion', {
            ...
            if(data != 0){
                ...
                obtenerDatos(tipoGrafica, dimGrafica, data.config);
    ...
    $('#graficar').on('click', function () {
        if ($('#formRE').valid() && $('#formRE2').valid()) {
            ...
            var configInicio = [];            
            obtenerDatos(tipoGrafica, dimGrafica, configInicio);
        }
    });
